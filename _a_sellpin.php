...
if (isset($_POST['user_id']) and isset($_POST['pins'])) {
	...
	if ($user_id<>"" and $pins <> "" and $pins >= 0) {
		...
		if ($mgr->id <= 3) {
			...
			$tt = $db->query("insert into tblpintran (idfrom,idto,efrom,eto,qty,trdate) values ($user->id,$mgr->id,'$user->username','$mgr->username',$pins,'$date')");
...
<body>
	<div class="easyui-panel" title="Sell PINs" style="width:600px;padding:10px;">
		...
		<?
			...
			$rs = $db->query("select * from tblpinlog order by id desc limit 50");
