...
function ggProcess1($id) {
    ...
    $timer = ggAddHours($now,48);

	$s_timer = ggDateToString($timer);
	//echo "Server Time: " . $s_now . " ". $s_timer."<br><br>";

	$rs = mysqli_fetch_object($db->query("select value from tblcounter where code = 'GIVE'"));
	$g_ctr = $rs->value;
...
//		$tem = $db->query("update tblmavro set date_release = '$s_now' where help_id=".$rec->id ." and op_type='B'");
	} else {
		$tem = $db->query("update tblhelp set status='C', g_pending=0, date_match = '$s_now' where id=".$rec->id);
	}
}
