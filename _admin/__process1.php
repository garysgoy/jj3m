...
if ($id > 0) {
	if ($id == 19640001) {
		$rs = mysqli_fetch_object($db->query("select * from tblhelp where g_type='P' and (status='O' or status='P') order by g_type, priority, g_date limit 1")) or die("err sel ".$db->error);
		$id = $rs->id;
