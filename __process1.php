...
if ($id > 0) {
	if ($id == 19640001) {
		if ($rs = mysqli_fetch_object($db->query("select * from tblhelp where g_type='P' and (status='O' or status='P') order by g_type, priority, g_date limit 1"))) {
			$id = $rs->id;
		}
	}
	if ($id == 19640001) {
		echo "No ID to process";
	} else if (ggCanProcess1($id)) {
		ggProcess1($id);
		echo "Processed Help_Id = $id";
	} else {
		echo "NOT ENOUGH GH TO PROCESS";
	}
} else if (isset($_REQUEST['date_end'])) {
	$date_end = $_REQUEST['date_end'];
	$rs = $db->query("select * from tblhelp where g_type='P' and g_date<= '$date_end 23:59:59' and priority<=$pr and note<>'referral' and note<>'manager' and (status='O' or status='P')  order by g_type, priority, g_date") or die("err sel ".$db->error);
	$rs1 = mysqli_fetch_object($db->query("select count(id) as ctr, sum(g_amount) as amt from tblhelp where g_type='P' and g_date<= '$date_end 23:59:59' and note<>'referral' and note<>'manager' and priority<=$pr and (status='O' or status='P')  order by g_type, priority, g_date")) or die("err sel ".$db->error);
...
function ggProcess1($id) {
	...
	while ($row = mysqli_fetch_object($db->query("select * from tblhelp where g_type='$second' and status = 'P' and note<>'referral' and note<>'manager' and mem_id <> $mem_id order by priority,id limit 1")) and $bal > 0) {
	...
	$ids = array();
	// Process Open
	while ($row = mysqli_fetch_object($db->query("select * from tblhelp where g_type='$second' and status = 'O' and note<>'referral' and note<>'manager' and mem_id <> $mem_id order by g_type,priority,g_date limit 1")) and $bal > 0) {
		if ($row->g_pending > $bal) {
			$res1 = $db->query("insert into tblhelpdetail (help_id, tran_id, mem_id, g_type, oth_id, g_date, g_amount,g_timer,status) VALUES ($gID, $g_ctr, $mem_id,'$prime',$row->mem_id,'$s_now',$bal,'$s_timer',0)") or die('Error 1: ' . $db->error);
			$res1 = $db->query("insert into tblhelpdetail (help_id, tran_id, mem_id, g_type, oth_id, g_date, g_amount,g_timer,status) VALUES ($row->id, $g_ctr, $row->mem_id,'$second',$mem_id,'$s_now',$bal,'$s_timer',0)") or die('Error 2: ' . $db->error);
			$res = $db->query("update tblhelp set g_pending = g_pending - $bal, status='P' where id =". $row->id ) or die('Error 2: ' . $db->error);
