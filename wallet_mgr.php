<?
	...
	$ls->wallet5 = array("Level Bonus","經理花紅","經理花紅");
	...
	$rs = $db->query("select count(*) from tblmavro where mem_id=$mem and wallet>2 and op_type='M' and type<>'X'");
	$ro = mysqli_fetch_row($rs);
	$result["total"] = $ro[0];
	$rs = $db->query("select * from tblmavro where mem_id=$mem and wallet>2 and op_type='M' and type <> 'X' order by $sort $order limit $offset,$rows");
	...
	while($row = mysqli_fetch_object($rs)){
		...
		if ($row->wallet == 1) {
			...
			if ($row->wallet == 6) {
				...
				$row->wallet = $wallet[4] . ' L'. ($row->op_level);
			}
		}
