...
$rs = $db->query("select h.*,m.email,m.username,m.phone,m.referral,m.manager from tblmember as m, tblhelp as h where h.mem_id = m.id and (m.manager = $id or m.referral = $id) and (g_type='P') and (h.status='O' or h.status='P' or h.status='C' or h.status='D') order by $sort $order limit $offset,$rows") or die("err ".$db->error);
$ctr = mysqli_fetch_array($db->query("select count(h.id) as ctr from tblmember as m, tblhelp as h where h.mem_id = m.id and (m.manager = $user->id or m.referral = $user->id) and (g_type='P') and (h.status='O' or h.status='P' or h.status='C' or h.status='D')")) or die("Err ". $db->error);
